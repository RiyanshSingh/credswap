...
CREATE POLICY "Buyers can create offers" ON marketplace_offers FOR INSERT
WITH CHECK (auth.uid() = buyer_id);

-- Buyer can view their own offers
DROP POLICY IF EXISTS "Buyers can view own offers" ON marketplace_offers;
CREATE POLICY "Buyers can view own offers" ON marketplace_offers FOR SELECT
USING (auth.uid() = buyer_id);

-- Seller can view offers on their items
DROP POLICY IF EXISTS "Sellers can view offers" ON marketplace_offers;
CREATE POLICY "Sellers can view offers" ON marketplace_offers FOR SELECT
USING (
    EXISTS (
...
DROP POLICY IF EXISTS "Sellers can update offers" ON marketplace_offers;
CREATE POLICY "Sellers can update offers" ON marketplace_offers FOR UPDATE
USING (
    EXISTS (
        SELECT 1 FROM marketplace_items mi
        WHERE mi.id = item_id AND mi.seller_id = auth.uid()
...
DROP FUNCTION IF EXISTS notify_seller_on_offer;
...
DROP FUNCTION IF EXISTS notify_buyer_on_offer_update;
