...
DROP FUNCTION IF EXISTS public.admin_delete_marketplace_item;
...
DROP FUNCTION IF EXISTS public.admin_update_marketplace_status;
...
DROP FUNCTION IF EXISTS public.admin_delete_lost_found_item;
...
DROP FUNCTION IF EXISTS public.admin_update_lost_found_status;
...
DROP FUNCTION IF EXISTS public.admin_get_opportunities;
...
DROP FUNCTION IF EXISTS public.admin_delete_opportunity;
...
DROP FUNCTION IF EXISTS public.admin_get_opportunity_applications;
...
DROP FUNCTION IF EXISTS public.admin_update_opportunity_application_status;
...
DROP FUNCTION IF EXISTS public.admin_delete_opportunity_application;
...
DROP FUNCTION IF EXISTS public.admin_delete_event;
...
-- Attendance & Scanner
DROP FUNCTION IF EXISTS public.get_admin_event_data;
...
DROP FUNCTION IF EXISTS public.mark_admin_notification_read;
...
DROP FUNCTION IF EXISTS public.clear_admin_notifications;
...
DROP FUNCTION IF EXISTS public.post_admin_message;
CREATE OR REPLACE FUNCTION public.post_admin_message(p_dispute_id UUID, p_content TEXT)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    IF NOT public.is_admin() THEN RAISE EXCEPTION 'Unauthorized'; END IF;
    INSERT INTO public.dispute_messages (dispute_id, sender_id, content_message)
...
CREATE POLICY "Everyone can view active events" ON public.events FOR SELECT USING (true);
...
CREATE POLICY "Admins can manage all registrations" ON public.event_registrations FOR ALL TO authenticated 
USING (public.is_admin()) WITH CHECK (public.is_admin());

DROP POLICY IF EXISTS "Users can manage own registrations" ON public.event_registrations;
CREATE POLICY "Users can manage own registrations" ON public.event_registrations FOR ALL TO authenticated 
USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

-- 4. CLEANUP OLD LOGINS & PROMOTION
-- Ensure riyansh@campulsy.in is the master admin
UPDATE public.profiles SET role = 'admin' WHERE email = 'riyansh@campulsy.in';
UPDATE public.profiles SET role = 'admin' WHERE email = 'itsyourriyansh@gmail.com';
UPDATE public.profiles SET role = 'admin' WHERE full_name ILIKE '%Riyansh%';

-- Remove admin status from generic accounts
UPDATE public.profiles SET role = 'student' WHERE email = 'admin@example.com';
