...
-- These were created by security_hardening.sql and are likely causing the "Login Failed" 
-- because the app can't read the profile after signing in.
...
BEGIN
    ...
    VALUES (v_user_id, v_email, 'Admin', 'admin', true)
    ON CONFLICT (id) DO UPDATE 
    SET role = 'admin', is_admin = true, email = v_email;
...
BEGIN
    ...
    -- Promote Profile
    INSERT INTO public.profiles (id, email, full_name, role, is_admin) VALUES (v_user_id, p_email, 'Admin', 'admin', true) ON CONFLICT (id) DO UPDATE SET role = 'admin', is_admin = true;
